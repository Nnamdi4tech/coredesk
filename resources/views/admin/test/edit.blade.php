...
<div class="w-full px-6 py-6 mx-auto">
    ...
    <div class="flex flex-wrap items-center justify-between mb-6 -mx-3">
        <div class="px-3">
            <h3 class="text-2xl font-bold text-slate-700 mb-1">Edit Test Entry</h3>
    ...
    <div class="bg-white shadow-soft-xl rounded-2xl overflow-hidden">
        ...
        <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
            ...
            <div>
                <h6 class="font-bold text-slate-700 mb-0">Edit Test Entry</h6>
                <p class="text-xs text-slate-400 mt-0.5">Update test information</p>
            </div>
        </div>

        <form method="POST" action="{{ route('tenant.test.update', [$subdomain, $test->id]) }}" class="p-6">
            @csrf
            @method('POST')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="class_id" required
                        ...
                        @foreach($classes as $class)
                            <option value="{{ $class->id }}" {{ $test->class_id == $class->id ? 'selected' : '' }}>
                                {{ $class->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="term" required
                        ...
                        <option value="1st" {{ $test->term == '1st' ? 'selected' : '' }}>1st Term</option>
                        <option value="2nd" {{ $test->term == '2nd' ? 'selected' : '' }}>2nd Term</option>
                        <option value="3rd" {{ $test->term == '3rd' ? 'selected' : '' }}>3rd Term</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="text"
                           name="session"
                           value="{{ old('session', $test->session) }}"
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-600 mb-1.5">
                        <i class="fa fa-pen mr-1 text-slate-400"></i> Test Type <span class="text-red-500">*</span>
                    </label>
                    <input type="text"
                           name="type"
                           value="{{ old('type', $test->type) }}"
                ...
                <div>
                    ...
                    <select name="teacher_id" required
                        ...
                        @foreach($teachers as $teacher)
                            <option value="{{ $teacher->id }}" {{ $test->teacher_id == $teacher->id ? 'selected' : '' }}>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="time"
                               name="start_time"
                               value="{{ old('start_time', $test->start_time) }}"
                               required
                               class="text-sm w-full rounded-lg border border-gray-300 bg-white py-2.5 px-3 text-gray-700 focus:outline-none focus:border-fuchsia-300 focus:ring-1 focus:ring-fuchsia-200 transition-all">

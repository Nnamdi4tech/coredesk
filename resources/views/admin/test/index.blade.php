...
<div class="w-full px-6 py-6 mx-auto">
    ...
    <div class="flex flex-wrap items-center justify-between mb-6 -mx-3">
        <div class="px-3">
            ...
            <p class="text-sm text-slate-400">
                ...
                <span class="text-slate-600">Test Management</span>
            </p>
        </div>
        
        <div class="px-3 mt-3 md:mt-0 flex gap-2">
            <button onclick="printTestSchedule()"
                ...
                <i class="fa fa-print mr-1"></i> Print Test Schedule
            </button>
            <a href="{{ route('tenant.test.create', $subdomain) }}"
               class="px-4 py-2 text-sm font-semibold text-white rounded-lg bg-gradient-to-tl from-green-600 to-emerald-500 shadow-soft-md hover:shadow-soft-xl hover:scale-105 transition-all">
                <i class="fa fa-plus mr-1"></i> Add New Test
            </a>
        </div>
    </div>
    ...
    <div class="bg-white shadow-soft-xl rounded-2xl p-4 mb-6">
        <div class="flex items-center justify-between mb-3 pb-2 border-b border-gray-100">
            <div class="flex items-center gap-2">
                ...
                <div>
                    <h6 class="font-bold text-slate-700 text-sm">Filter Tests</h6>
        ...
        <div class="flex flex-wrap items-end gap-2">
            ...
            <div class="flex-1 min-w-[160px]">
                <select id="filterClass"
                    ...
                    @foreach($tests->pluck('class.name')->unique()->filter()->sort() as $className)
            ...
            <div class="flex-1 min-w-[160px]">
                <select id="filterSubject"
                    ...
                    @foreach($tests->pluck('subject.name')->unique()->filter()->sort() as $subjectName)
            ...
            <div class="flex-1 min-w-[160px]">
                <select id="filterTeacher"
                    ...
                    @foreach($tests->pluck('teacher.name')->unique()->filter()->sort() as $teacherName)
        ...
        <div class="flex justify-between items-center mt-3 pt-2 border-t border-gray-100">
            <p class="text-xs text-slate-400">
                <i class="fa fa-info-circle mr-1 text-amber-400"></i>
                To print a specific class, subject, or teacher — select a filter first, then click <strong class="text-slate-600">Print Test Schedule</strong>.
            </p>
            <p class="text-xs text-slate-400">
                Showing <span id="visibleCount" class="font-bold text-slate-600">{{ $tests->count() }}</span> of {{ $tests->count() }} entries
            </p>
        </div>
    </div>

    {{-- Test Card --}}
    <div class="bg-white shadow-soft-xl rounded-2xl overflow-hidden" id="testCard">
        ...
        <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
            ...
            <div>
                <h6 class="font-bold text-slate-700 mb-0">Test Entries</h6>
                <p class="text-xs text-slate-400 mt-0.5">{{ $tests->count() }} total entries</p>
            </div>
        </div>
        ...
        @if($tests->isEmpty())
            <div class="p-12 text-center">
                ...
                <p class="text-slate-500 font-semibold text-lg">No test entries found</p>
                <p class="text-slate-400 text-sm mt-1">Click the "Add New Test" button to create your first test entry.</p>
            </div>
        @else
            ...
            <div class="">
                <table class="items-center w-full mb-0 align-top border-gray-200 text-slate-500" id="testTable">
                    ...
                    <tbody id="testBody">
                        ...
                        @php
                            $typeColors = [
                                'Quiz' => 'bg-pink-50 text-pink-600',
                                'Test' => 'bg-purple-50 text-purple-600',
                                'Examination' => 'bg-orange-50 text-orange-600',
                            ];
                            $typeColor = $typeColors[$t->type] ?? 'bg-gray-50 text-gray-600';
                        @endphp
                        <tr class="test-row border-t border-gray-100 hover:bg-gray-50 transition-colors"
                            ...
                            <td class="px-6 py-4 align-middle text-center">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <span class="text-sm text-slate-600">
                                        {{ $t->start_time ? \Carbon\Carbon::parse($t->start_time)->format('h:i A') : 'N/A' }}
                                        -
                                        {{ $t->end_time ? \Carbon\Carbon::parse($t->end_time)->format('h:i A') : 'N/A' }}
                                    </span>
                                </div>
                            </td>
                            <td class="px-6 py-4 align-middle text-center">
                            ...
                            <td class="px-6 py-4 align-middle text-center no-print">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('tenant.test.edit', [$subdomain, $t->id]) }}"
                                    ...
                                    <a href="{{ route('tenant.test.delete', [$subdomain, $t->id]) }}"
                                       onclick="return confirm('Are you sure you want to delete this test entry?')"
    ...
    <div class="mt-5 p-4 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl border border-blue-100 no-print">
        <div class="flex items-start gap-3">
            ...
            <div>
                <p class="text-sm font-semibold text-blue-800">Test Management</p>
                <p class="text-xs text-blue-600 mt-0.5">
                    Add, edit, or remove class test entries. Each entry represents a subject test scheduled for a specific class on a particular date and time.
                </p>
            </div>
        </div>
    </div>
...
function printTestSchedule() {
    ...
    if (visibleRows.length === 0) {
        alert('No test entries to print. Please adjust your filters.');
        return;
    }
    ...
    visibleRows.forEach(row => {
        ...
        grouped[cls].push({
            ...
            session: row.dataset.session || 'N/A',
        });
    });
    ...
    let html = `
        <div class="print-header">
            <h2>${schoolName} — Test Schedule</h2>
            <p>Printed on ${now}</p>
        </div>
    `;

...
<div class="w-full px-6 py-6 mx-auto">
    ...
    <div class="flex flex-wrap items-center justify-between mb-6 -mx-3">
        <div class="px-3">
            <h3 class="text-2xl font-bold text-slate-700 mb-1">Add Test Date</h3>
    ...
    <div class="bg-white shadow-soft-xl rounded-2xl overflow-hidden">
        ...
        <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
            <div class="w-8 h-8 rounded-lg bg-gradient-to-tl from-purple-500 to-pink-400 flex items-center justify-center shadow-soft-md">
                <i class="fa fa-plus-circle text-black text-xs"></i>
            </div>
            <div>
                <h6 class="font-bold text-slate-700 mb-0">New Test Entry</h6>
                <p class="text-xs text-slate-400 mt-0.5">Schedule a test for a specific class</p>
            </div>
        </div>

        <form method="POST" action="{{ route('tenant.test.store', $subdomain) }}" class="p-6">
            @csrf
...
<div>
    <label class="block text-xs font-semibold text-slate-600 mb-1.5">
        <i class="fa fa-calendar-week mr-1 text-slate-400"></i> Test Type <span class="text-red-500">*</span>
    </label>
    <input type="text" 
           name="type" 
           value="{{ old('type') }}"
           placeholder="e.g. 1st CA, 2ND CA, or 1st Semester"
           required
           class="text-sm w-full rounded-lg border border-gray-300 bg-white py-2.5 px-3 text-gray-700 focus:outline-none focus:border-fuchsia-300 focus:ring-1 focus:ring-fuchsia-200 transition-all">
    ...
    @error('date') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
            ...
            <div class="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
                ...
                <button type="submit"
                    ...
                    <i class="fa fa-save mr-1"></i> Save Test Entry
                </button>
            </div>
        </form>
    </div>
